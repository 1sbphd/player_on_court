...
from player_on_court import __version__
from player_on_court import player_on_court
...
def test_players_start_quater(ten_players_period):
    start_players = player_on_court.adding_player_on_court(ten_players_period)\
...
def test_sort_players(ten_players_period):
    unsort_pl = player_on_court.player_on_court(ten_players_period)
    sort_pl = player_on_court.sort_players(ten_players_period, unsort_pl)
    assert sort_pl == [202954, 1627737, 1629673, 203952, 1629672,
                       1627752, 1627747, 1629013, 201145, 1628386]


def test_nine_players(nine_players_period):
    player_id = player_on_court.player_on_court(nine_players_period)
    assert (202066 not in player_id) and (len(player_id) == 9)


def test_lost_internet(nine_players_period):
    with pytest.raises(ConnectionError):
        player_on_court.adding_player_on_court(nine_players_period,
                                               timeout=0.00001)


def test_eleven_players(eleven_players_period):
    player_id = player_on_court.player_on_court(eleven_players_period)
    assert (202326 in player_id) and (len(player_id) == 11)


def test_cashe():
    info1 = player_on_court._cache_player_info()
    info2 = player_on_court._cache_player_info()
...
def test_fill_list_array(ten_players_period, all_id, shape):
    df = player_on_court.fill_columns_data(ten_players_period, all_id)
    assert df.shape == shape


def test_replace(ten_players_period):
    df = player_on_court.adding_player_on_court(ten_players_period)
    pl_list = player_on_court\
        .replace_id_on_name(df.iloc[0, 34:].reset_index(drop=True))
