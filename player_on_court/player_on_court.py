...
def player_on_court(data: pd.DataFrame, all_id: Optional[np.ndarray] = None) -> np.ndarray:
...
def fill_columns_data(data: pd.DataFrame, all_id: Union[np.ndarray, List]) -> pd.DataFrame:
...
def adding_player_on_court(data: pd.DataFrame, **kwargs: Dict[str, float]) -> pd.DataFrame:
    ...
    for period in data["PERIOD"].unique():
        ...
        all_id = player_on_court(df)
        if len(all_id) == 10:
            all_id = sort_players(df, all_id)
            d[period] = fill_columns_data(df, all_id)
        else:
            retry = 0
            bx = ""
            ...
            all_id = player_on_court(df, all_id)
            d[period] = fill_columns_data(df, all_id)
...
def replace_id_on_name(id_players: Union[pd.Series, np.ndarray, List],
...
#     check_tbl = pd.read_csv("~/R/projects/player_on_floor/check_data.csv")
#     check_tbl1 = pd.DataFrame(columns=["SEASON", "GAME_ID", "PERIOD", "END_PERIOD", "PLAYER1", "PLAYER2", "PLAYER3",
#                                        "PLAYER4", "PLAYER5", "PLAYER6", "PLAYER7", "PLAYER8", "PLAYER9", "PLAYER10"])
#     player_info = pd.read_csv("~/R/projects/experimet_dff_shot/datasets/commonal_players.csv")
#     for row in check_tbl.itertuples():
#         if row.Index % 2 == 0:
#             period = row.PERIOD
#             season = row.SEASON
#             game_id = int(str(row.GAME_ID)[-4:])
#             tbl = pd.read_csv(
#                 "~/env/nba/data/" + str(season) + "/nbastats/" + str(season) + "_" + str(game_id) + ".csv")
#             tbl = tbl.iloc[:, :34]
#             tbl = tbl.loc[tbl["PERIOD"] == period].reset_index(drop=True)
#             tbl = adding_player_on_court(tbl)
#             tbl = tbl.loc[:, ["PLAYER1", "PLAYER2", "PLAYER3", "PLAYER4", "PLAYER5",
#                               "PLAYER6", "PLAYER7", "PLAYER8", "PLAYER9", "PLAYER10"]]
#             first_row = tbl.iloc[0]
#             last_row = tbl.iloc[-1]
#             first_r = pd.Series([row.SEASON, row.GAME_ID, row.PERIOD, 0],
#                                 index=["SEASON", "GAME_ID", "PERIOD", "END_PERIOD"]).append(first_row)
#             last_r = pd.Series([row.SEASON, row.GAME_ID, row.PERIOD, 1],
#                                index=["SEASON", "GAME_ID", "PERIOD", "END_PERIOD"]).append(last_row)
#             check_tbl1 = check_tbl1.append(first_r, ignore_index=True)
#             check_tbl1 = check_tbl1.append(last_r, ignore_index=True)
#         else:
#             continue
#
#     n1 = np.sort(check_tbl.loc[:, ["PLAYER1", "PLAYER2", "PLAYER3", "PLAYER4", "PLAYER5"]].to_numpy(), axis=1)
#     n2 = np.sort(check_tbl1.loc[:, ["PLAYER1", "PLAYER2", "PLAYER3", "PLAYER4", "PLAYER5"]].to_numpy(), axis=1)
#     n3 = np.sort(check_tbl.loc[:, ["PLAYER6", "PLAYER7", "PLAYER8", "PLAYER9", "PLAYER10"]].to_numpy(), axis=1)
#     n4 = np.sort(check_tbl1.loc[:, ["PLAYER6", "PLAYER7", "PLAYER8", "PLAYER9", "PLAYER10"]].to_numpy(), axis=1)
#
#     assert np.all(np.all(n1 == n2, axis=1))
#     assert np.all(np.all(n3 == n4, axis=1))
#     assert replace_id_on_name(np.array([201939, 202691, 201142, 2738, 1626172, 1629012, 202684, 203918, 201588,
#                                         1626224]),
#                               player_info) == ['Stephen Curry', 'Klay Thompson', 'Kevin Durant', 'Andre Iguodala',
#                                                'Kevon Looney', 'Collin Sexton', 'Tristan Thompson', 'Rodney Hood',
#                                                'George Hill', 'Cedi Osman']
